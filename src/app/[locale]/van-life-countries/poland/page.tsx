...
const HERO_IMAGE_SRC = "/images/van-life/gdansk/2025-07-05 15.10.08_GDansk_4096.jpg";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "vanLifeCountries.poland" });
...
export default async function PolandPage({ params }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "vanLifeCountries.poland" });
  const tCommon = await getTranslations({ locale, namespace: "vanLifeCountries.common" });
          ...
          <div className="space-y-6 pb-8">
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
              <div className="overflow-hidden rounded-3xl">
                <iframe
                  src="https://share.trackiwi.com/a8j7Z3IlkZM"
                  width="100%"
                  height="500"
                ></iframe>
              </div>
