...
const HERO_IMAGE_SRC = "/images/van-life/oostende/2025-06-14 21.46.36_Oostende_4096.jpg";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "vanLifeCountries.belgium" });
...
export default async function BelgiumPage({ params }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "vanLifeCountries.belgium" });
  const tCommon = await getTranslations({ locale, namespace: "vanLifeCountries.common" });
          ...
          <div className="space-y-6 pb-8">
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
              <div className="overflow-hidden rounded-3xl">
                <iframe
                  src="https://share.trackiwi.com/s4DLzWRqLV4"
                  width="100%"
                  height="500"
                ></iframe>
              </div>
