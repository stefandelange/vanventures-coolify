...
export async function generateStaticParams() {
  const postsEn = await getPostsByCollection("en", "van-life");
  const postsDe = await getPostsByCollection("de", "van-life");

  return [
    ...postsEn.map((post) => ({ locale: 'en', slug: post.slug })),
    ...postsDe.map((post) => ({ locale: 'de', slug: post.slug })),
  ];
}
...
export async function generateMetadata({
  ...
  try {
    ...
    const post = await getPostBySlugAndLocale(slug, locale, "van-life");
    return {
      title: post.title,
...
export default async function PostPage({ params }: PostPageProps) {
  ...
  let post;
  try {
    post = await getPostBySlugAndLocale(slug, locale, "van-life");
  } catch {
    return notFound();
  }
  ...
  return (
    <>
      <article id="post-top" className="bg-white">
        ...
        <section className="mx-auto flex max-w-3xl flex-col gap-10 px-6 py-16 sm:px-0">
          ...
          <ContentLanguageNotice isFallback={post.isFallback} requestedLocale={locale} />
